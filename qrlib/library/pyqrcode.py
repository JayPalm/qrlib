...
class CodeLengthOverflowError(Exception):
    ...
    def __str__(self):
        return "CodeLengthOverflowError(bits=%d,maxbits=%d)" % (
            self.bits,
            self.maxbits,
        )
...
def MakeQR(
    data, minTypeNumber=0, errorCorrectLevel=QRErrorCorrectLevel.Q, verbose=False
):
    ...
    for x in range(0, 50):
        try:
            ...
            if VERBOSE:
                print("QRCode.Make - bad guess - trying again", x, file=sys.stderr)
...
class QRCode(object):
    ...
    def make_image(
        ...
        pixelsize = (
            self.getModuleCount() + border_in_blocks + border_in_blocks
        ) * block_in_pixels
    ...
    def createData(typeNumber, errorCorrectLevel, dataList):
        ...
        if buffer.getLengthInBits() > totalDataCount * 8:
            raise CodeLengthOverflowError(
                bits=buffer.getLengthInBits(), maxbits=totalDataCount * 8
            )
