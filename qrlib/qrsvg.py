...
def _5plus_touching(
    ...
    raise Exception("Five or more touching node, none in cross section," + "impossible")
...
def _within_inner_eye(row, column, qr_size):
    if row >= 2 and row <= 4 and column >= 2 and column <= 4:
        ...
        return True
    if row >= qr_size - 6 and row <= qr_size - 3 and column >= 2 and column <= 4:
        # In the bottom left inner eye
        return True
    if row >= 2 and row <= 4 and column >= (qr_size - 6) and column <= (qr_size - 3):
...
def _qrcode_to_svg(
    ...
    # Background square
    et.SubElement(
        svg_doc, "rect", x="0", y="0", width=width, height=height, fill=bg_color
    )
    ...
    for row in range(module_count):
        for column in range(module_count):
            ...
            if outer_eyes_dict and _is_outer_eye_position(row, column, module_count):
                _insert_shape(
                    column,
                    row,
                    ("outer.svg", 0),
            ...
            if inner_eyes_dict and _is_inner_eye_position(row, column, module_count):
                _insert_shape(
                    column,
                    row,
                    ("inner.svg", 0),
            ...
            if outer_eyes_dict and _within_outer_eye(row, column, module_count):
                continue

            if inner_eyes_dict and _within_inner_eye(row, column, module_count):
                continue

            shape = _choose_module(
                top_left=top_left,
                top_center=top_center,
                top_right=top_right,
                middle_left=middle_left,
...
def generate_QR_for_text(
    ...
    qr_code = pyqrcode.MakeQR(text, errorCorrectLevel=string_to_eclevel(ec_level))
    return _qrcode_to_svg(
        qr_code,
        style=style,
