...
def _gen_pdf(qr_pil, instructions=True, bg_color="#FFFFFF", frame=True, put_logo=True):
    """
    ...
    if frame:
        ...
        qr_canvas.rect(
            x=sq_draw_x, y=sq_draw_y, width=sq_width, height=sq_height, fill=1
        )
        ...
        qr_canvas.drawImage(SCISSORS_IMAGE_PATH, x=scissors_x, y=sq_draw_y, mask="auto")
...
def _generate_pil(
    ...
    generated_svg = qrsvg.generate_QR_for_text(
        text,
        size=size,
        ec_level=ec_level,
        style=style,
...
def _gen_filelike(
    ...
    if qr_format == "SVG":
        return qrsvg.generate_QR_for_text(
            text,
            size=size,
            ec_level=ec_level,
            style=style,
    ...
    if qr_format in ["GIF", "JPEG", "PNG"]:
        ...
        raise Exception(
            "Awkward, unrecognised qr_format "
...
def generate_qr_file(
    ...
    if app == "interior":
        ...
        if app_size == "small":
            ...
            raise Exception(
                "No app size defined for QR generation, "
                + "looks like validation failed. Awkward!"
            )
    else:
        raise Exception(
            "No app type defined for QR generation, looks like"
            + " validation failed. Awkward!"
        )

    return _gen_filelike(
        text,
        size=size,
        ec_level=ec_level,
        qr_format=qr_format,
...
def generate_custom_qr_file(
    ...
    if qr_format == "PDF" and size > EXTERIOR_LARGE["size"]:
        raise Exception(
            "size cannot be > " + "%s when generating a PDF" % (EXTERIOR_LARGE["size"])
        )
